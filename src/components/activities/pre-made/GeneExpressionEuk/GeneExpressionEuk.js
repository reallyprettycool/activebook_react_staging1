...
class GeneExpressionEuk extends React.Component {
  constructor(props) {
    super(props);
    this.state = { 
  ...
  moveMolecules(movablesArray) {
    let movablesArrayB = movablesArray.slice();
    // const len = movablesArrayB.length;
    for (let item of movablesArrayB) {
      switch (item.name) {
        case 'polymerase':
          if (item.x < 750) { // (item.x < 800)
            if (item.y < 300) { item.y += 1.5}
            if (item.x < 500 && item.x + item.vx >= 500 ) { // polymerase reached 
              ...
              movablesArrayB.unshift(RNA); // add RNA into movable array
            }
        ...
        case 'rna' :
          if (item.toX < 800) {
            ...
            item.fromY -= item.vy;
            if (item.x%10 === 0) {
              // movablesArrayB.splice( len - 3, 1) // remove a moving nucleotide
            }
          }
          break;
        case 'ribosome':
          if (item.x < 750) { // ribosome moves from 450 - 750, breakpoints: 550, 650
            if (item.x < 600 && item.x + item.vx >= 600) {
              movablesArrayB.pop(); // remove polymerase from movable array
            }
            if (item.x%10 === 0 ) {
              // movablesArrayB.splice( len - 3, 1) // remove a moving aminoacid
              ...
              movablesArrayB.unshift(aminoacid) // add new aminoacid in peptide to beginning of array
              // console.log('new amino acid', movablesArrayB)
            }
            if ((item.x < 550 && item.x + item.vx >= 550)) {
              let name = 'Polypeptide';
              const label = {
                type: 'text',
                x: item.x*0.75 + 150,
                y: item.x * 0.5 - 190,
              ...
              movablesArrayB.unshift(label)
            }
            
            item.x += item.vx; // move at a speed of 2 on the x axis
          } else {
            this.setState({animation: false})
            // console.log('movables' , movablesArrayB)
            this.checkIfWon(); // move to next level
          }
          break;
        case 'RNA nucleotides':
          if (item.x + item.vx > 840 || item.x + item.vx < 20) { item.vx *= -1;}
          if (item.y + item.vy > 340 || item.y + item.vy < 205) { item.vy *= -1;}
          item.x += item.vx;
          item.y += item.vy;
          break;
        case 'aminoacid':
          if (item.x + item.vx > 850 || item.x + item.vx < 7) { item.vx *= -1;}
          if (item.y + item.vy > 195 || item.y + item.vy < 60) { item.vy *= -1;}
          item.x += item.vx;
          item.y += item.vy;
          break;
        default: 
  ...
  myDown = (mx, my) => {
    ...
    for (let r of draggableArray) {
      ...
      if (r.type === 'intron') { y = r.y - 10} // offset the start of text
      // console.log( mx, r.x, r.x + r.width, my,  y,  y + r.height,( mx > r.x && mx < r.x + r.width && my > y && my < y + r.height))
...
export default GeneExpressionEuk