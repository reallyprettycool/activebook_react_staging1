...
class Canvas extends React.Component {
  constructor(props) {
    ...
    drawFixtures () {
      for (let fixture of this.props.fixtures) {
        switch (fixture.type) {
          case 'chromosome':
            this.drawChromosomes(fixture.x, fixture.y, fixture.size, fixture.arms, fixture.origin); 
            break;    
          case 'phases': 
          ...
          case 'cell':
            ...
            break;    
          case 'box':
            if (this.props.level < 5) { // do not draw boxes for level 5    
              this.ctx.beginPath();
    ...
    drawDraggables () {
      this.ctx.beginPath();
      this.ctx.rect(700, 20, 285, 355);
      this.ctx.closePath();
      this.ctx.fillStyle = '#e5e5e5';
      this.ctx.fill();
      this.ctx.font = 'bold 20px sans-serif';
      this.ctx.fillStyle = 'black';
      this.ctx.fillText('Drag these:', this.canvas.width*0.80, 70);

      for (let draggable of this.props.draggables) {
        switch (draggable.type) {
          case 'chromosome':
            this.drawChromosomes(draggable.x, draggable.y, draggable.size, draggable.arms, draggable.origin); 
            break;    
          case 'phases': 
            this.ctx.font = 'bold 20px sans-serif';
            this.ctx.fillStyle = 'black';
  ...
  render() {
    ...
    <div className='canvas-container'>
      <canvas id='water-container' height="450px" width="1000px" ref={this.canvasRef} />
    </div>
    )
  }
}
